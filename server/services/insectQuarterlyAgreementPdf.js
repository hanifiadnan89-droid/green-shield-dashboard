...
/** Landscape letter: 11in × 8.5in — same as Bed Bug. */
export const IQ_PAGE_SIZE = { width: 792, height: 612 };
...
/**
 * Format payment text for Insect Quarterly calendar tiles (red (S) on service months).
 */
export function formatInsectQuarterlyPaymentText(month) {
...
export function buildInsectQuarterlySchedule(params = {}) {
...
export function normalizeInsectQuarterlyAgreementData(input = {}) {
  const {
    lead = {},
    pricing = {},
    address = {},
...
async function drawHeader(pdfDoc, page, fonts) {
  ...
  const titleSize = 11;
  const titleWidth = fonts.bold.widthOfTextAtSize(IQ_TITLE, titleSize);
  const titlePadX = 12;
  const titlePadY = 6;
...
function drawPestsSection(page, data, fonts, pestImages) {
  ...
  const x = MARGIN_X;
  const y = yFromTop(top, h);
  drawBubblePanel(page, { x, y, w, h, title: IQ_COVERED_PESTS_SECTION_TITLE, font: fonts.bold });
...
function drawMiddleRow(page, schedule, fonts) {
  ...
  months.forEach((month, index) => {
    ...
    drawPaymentTile(page, month.label, formatInsectQuarterlyPaymentText(month), {
      ...
      font: fonts.regular,
      fontBold: fonts.bold,
      tileStyle: IQ_CALENDAR_TILE_STYLE,
    });
  });
}
...
function drawAuthorizationSection(page, fonts) {
  ...
  const x = MARGIN_X;
  const y = yFromTop(top, h);
  drawBubblePanel(page, { x, y, w, h, title: IQ_AUTHORIZATION_TITLE, font: fonts.bold });
  drawWrappedText(page, IQ_AUTHORIZATION_TEXT, {
    x: x + 6,
    y: bodyStartY(y, h),
    w: w - 12,
    font: fonts.regular,
...
function drawSignatureSection(page, data, fonts, signatureAssets = {}) {
  ...
  drawUnderlinedLabel(page, {
    x: periodX,
    y: periodY,
    text: IQ_AGREEMENT_PERIOD_TEXT,
    size: periodSize,
    font: fonts.bold,
    color: COLORS.headerBg,
  });

  drawWrappedText(page, IQ_INITIALS_TEXT, {
    x: x + SECTION_PAD,
...
/**
 * Build a vector Insect Quarterly agreement PDF (Bed Bug design system).
 */
export async function buildInsectQuarterlyAgreementPdf(input = {}, options = {}) {
  const { signatures = null } = options;
  const data = normalizeInsectQuarterlyAgreementData(input);
  console.log('[iq-pdf] building vector agreement for', data.customerName || '(no name)');
  ...
  if (schedule.warning) {
    console.warn(`[iq-pdf] ${schedule.warning}`);
  }
  ...
  return {
    outBytes,
    outName: `${safeName}_Insect_Quarterly.pdf`,
    schedule,
    pageSize: { ...IQ_PAGE_SIZE },
    data,
  };
}
