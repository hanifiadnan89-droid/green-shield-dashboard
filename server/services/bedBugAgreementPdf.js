...
/**
 * Pre-cleaned artwork:
 * - real Green Shield logo
 * - updated phone number
 * - no Service Notes / Internal Notes row
 * - no hardcoded months
 */
...
/**
 * Calendar cells aligned to the template artwork.
 * monthY and payY use pdf-lib bottom-origin coordinates.
 */
const MONTH_CELLS = [
...
const PEST_ICON_PLACEMENTS = [
  { type: 'bed_bug', x: 29, y: 743 },
  { type: 'ant', x: 168, y: 743 },
  { type: 'ant', x: 307, y: 743 },
  { type: 'ant', x: 446, y: 743 },

  { type: 'bee', x: 29, y: 729 },
  { type: 'wasp', x: 168, y: 729 },
  { type: 'spider', x: 307, y: 729 },
  { type: 'bug', x: 446, y: 729 },

  { type: 'flea', x: 29, y: 715 },
  { type: 'centipede', x: 168, y: 715 },
  { type: 'cricket', x: 307, y: 715 },
  { type: 'silverfish', x: 446, y: 715 },

  { type: 'mouse', x: 29, y: 679 },
  { type: 'rat', x: 168, y: 679 },
  { type: 'tick', x: 307, y: 679 },
  { type: 'cockroach', x: 446, y: 679 },
];
...
function paymentFontSize(text) {
  ...
  if (length > 8) return 5.5;

  return 6.5;
}

/**
 * Build Bed Bug agreement from the professional template artwork
...
export async function buildBedBugAgreementPdf({
  ...
  for (const field of form.getFields()) {
    try {
      ...
      // Some fields may not be text fields or may not support these operations.
  ...
  function fill(name, value) {
    ...
    try {
      ...
      // Field may not exist on older/newer templates.
      // Ignore safely so generation does not crash.
    }
  }
  ...
  if (discVal > 0) {
    fill('initial_discount', `-${discVal.toFixed(2)}`);
  }
  ...
  /**
   * Service type / frequency values.
   * Labels are baked into the cleaned template.
   */
  eraseRect(page, {
    x: 402,
    y: 804,
  ...
  page.drawText(BED_BUG_SERVICE_TYPE, {
    x: 404,
    y: 812,
    size: 8,
    font: helveticaBold,
  ...
  page.drawText(BED_BUG_SERVICE_FREQUENCY, {
    x: 404,
    y: 784,
    size: 8,
    font: helvetica,
  ...
  MONTH_CELLS.forEach((cell, index) => {
    ...
    if (month?.paymentText) {
      drawCenteredText(page, month.paymentText, {
        cx,
        y: cell.payY,
        size: paymentFontSize(month.paymentText),
        font: helvetica,
      });
    }
  });
...
export const BED_BUG_PAGE_SIZE = {
  width: PAGE_W,
  height: PAGE_H,
};